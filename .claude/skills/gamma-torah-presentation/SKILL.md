...
### 4. שלח `inputText` **ללא ניקוד** — גמה מנקדת
עברית מנוקדת עולה פי 3-4 בטוקנים. לכן אל תנקד את הטקסט בעצמך: כתוב עברית רגילה
ב-`inputText`, והוסף ל-`additionalInstructions` הוראה לגמה לנקד את הכל (ראה כלל
הניקוד ב-Step 5). כך הניקוד המלא עדיין מגיע לילדים, אבל בלי לשלם עליו בטוקנים.
⚠️ הניקוד עצמו הוא דרישה פדגוגית מחייבת — מה שנחסך הוא רק **מי** מייצר אותו.
...
## Step 7: בנייה ישירה בגמה (Direct Build) — ברירת המחדל, אלא אם המשתמש ביקש אחרת ב-Step 0

אם המשתמש לא ביקש במפורש את מסלול העתקה-הדבקה הידני — בנה את המצגת ישירות בגמה:
...
3. ⛔ **אל תעקוב אחר הסטטוס בלולאה.** `generate` מחזיר `gammaUrl` מיד, והווידג'ט של גמה
   כבר מציג למשתמש את ההתקדמות. אל תריץ `get_generation_status` שוב ושוב ואל תשתמש
   ב-`Monitor`/`sleep` להמתנה (ראה כללי היעילות בראש הקובץ).
4. מסור למשתמש את ה-`gammaUrl` שהוחזר **מיד**, וסיים את התור. רק אם המשתמש ביקש במפורש
   את קישור ה-`/docs/` הסופי או את מספר הקרדיטים — קרא ל-`get_generation_status`
   **פעם אחת בלבד**.
...
אם המשתמש מבקש במפורש טקסט + פרומפטים להעתקה-הדבקה ידנית — עבור למסלול הקיים (Step 4-6),
